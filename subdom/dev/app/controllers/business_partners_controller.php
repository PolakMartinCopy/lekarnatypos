...
//			array('field' => 'StoreItem.last_sale_date', 'position' => '["StoreItem"]["last_sale_date"]', 'alias' => 'StoreItem.last_sale_date'),
		...
		if ($this->Acl->check(array('model' => 'User', 'foreign_key' => $this->Session->read('Auth.User.id')), 'controllers/CSInvoices/index')) {
			...
			if (isset($this->params['named']['reset']) && $this->params['named']['reset'] == 'c_s_invoices') {
				$this->Session->delete('Search.CSInvoiceForm2');
				$this->redirect(array('controller' => 'business_partners', 'action' => 'view', $business_partner['BusinessPartner']['id'], 'tab' => 14));
			}
			
			// pokud chci vysledky vyhledavani
			if ( isset($this->data['CSInvoiceForm2']['CSInvoice']['search_form']) && $this->data['CSInvoiceForm2']['CSInvoice']['search_form'] == 1 ){
				$this->Session->write('Search.CSInvoiceForm2', $this->data['CSInvoiceForm2']);
				$c_s_invoices_conditions = $this->BusinessPartner->CSInvoice->do_form_search($c_s_invoices_conditions, $this->data['CSInvoiceForm2']);
			} elseif ($this->Session->check('Search.CSInvoiceForm2')) {
				$this->data['CSCreditNoteForm2'] = $this->Session->read('Search.CSInvoiceForm2');
				$c_s_invoices_conditions = $this->BusinessPartner->CSInvoice->do_form_search($c_s_invoices_conditions, $this->data['CSInvoiceForm2']);
			}
			...
			$this->BusinessPartner->CSInvoice->CSProduct = new CSProduct;
			App::import('Model', 'Unit');
		...
		if ($this->Acl->check(array('model' => 'User', 'foreign_key' => $this->Session->read('Auth.User.id')), 'controllers/CSCreditNotes/index')) {
			...
			if (isset($this->params['named']['reset']) && $this->params['named']['reset'] == 'c_s_credit_notes') {
				$this->Session->delete('Search.CSCreditNoteForm2');
				$this->redirect(array('controller' => 'business_partners', 'action' => 'view', $business_partner['BusinessPartner']['id'], 'tab' => 15));
			}
			
			// pokud chci vysledky vyhledavani
			if ( isset($this->data['CSCreditNoteForm2']['CSCreditNote']['search_form']) && $this->data['CSCreditNoteForm2']['CSCreditNote']['search_form'] == 1 ){
				$this->Session->write('Search.CSCreditNoteForm2', $this->data['CSCreditNoteForm2']);
				$c_s_credit_notes_conditions = $this->BusinessPartner->CSCreditNote->do_form_search($c_s_credit_notes_conditions, $this->data['CSCreditNoteForm2']);
			} elseif ($this->Session->check('Search.CSCreditNoteForm2')) {
				$this->data['CSCreditNoteForm2'] = $this->Session->read('Search.CSCreditNoteForm2');
				$c_s_credit_notes_conditions = $this->BusinessPartner->CSCreditNote->do_form_search($c_s_credit_notes_conditions, $this->data['CSCreditNoteForm2']);
			}
			...
			// aby mi to radilo i podle poli modelu, ktere nemam primo navazane na delivery note, musim si je naimportovat
			App::import('Model', 'CSProduct');
			$this->BusinessPartner->CSCreditNote->CSProduct = new CSProduct;
			App::import('Model', 'Unit');
		...
		if ($this->Acl->check(array('model' => 'User', 'foreign_key' => $this->Session->read('Auth.User.id')), 'controllers/CSTransactions/index')) {
			...
			if (isset($this->params['named']['reset']) && $this->params['named']['reset'] == 'c_s_transactions') {
				$this->Session->delete('Search.CSTransactionForm2');
			...
			// pokud chci vysledky vyhledavani
			if ( isset($this->data['CSTransactionForm2']['CSTransaction']['search_form']) && $this->data['CSTransactionForm2']['CSTransaction']['search_form'] == 1 ){
				$this->Session->write('Search.CSTransactionForm2', $this->data['CSTransactionForm2']);
				$c_s_transactions_conditions = $this->BusinessPartner->CSTransaction->do_form_search($c_s_transactions_conditions, $this->data['CSTransactionForm2']);
			} elseif ($this->Session->check('Search.CSTransactionForm2')) {
				$this->data['CSTransactionForm2'] = $this->Session->read('Search.CSTransactionForm2');
				$sales_conditions = $this->BusinessPartner->CSTransaction->do_form_search($c_s_transactions_conditions, $this->data['CSTransactionForm2']);
			}
