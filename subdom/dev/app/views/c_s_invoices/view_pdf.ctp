...
$tcpdf->Cell(190, 0, 'Faktura č. ' . $invoice['CSInvoice']['code'], 0, 0, 'R', false);
...
if (!empty($invoice['BusinessPartner']['ico'])) {
	$ico_info = 'IČ: ' . $invoice['BusinessPartner']['ico'];
}

$dic_info = '';
if (!empty($invoice['BusinessPartner']['dic'])) {
	$dic_info = 'DIČ: ' . $invoice['BusinessPartner']['dic'];
}
...
foreach ($invoice['CSTransactionItem'] as $transaction_item) {
	$tbl .= "
	<tr>
		<td nowrap=\"nowrap\">" . $transaction_item['c_s_product_name'] . "</td>
...
$tcpdf->SetFont($textfont, '', 8);
$tcpdf->Cell(190, 0, 'Základ: ' . number_format($invoice['CSInvoice']['amount'], 2, ',', ' ') . ' Kč', 0, 1, 'R', false);
$tcpdf->Cell(190, 0, 'Daň: ' . number_format($invoice['CSInvoice']['amount_vat'] - $invoice['CSInvoice']['amount'], 2, ',', ' ') . ' Kč', 0, 1, 'R', false);
$tcpdf->SetFont($textfont, 'B', 8);
$tcpdf->Cell(190, 0, 'Celkem k úhradě: ' . number_format($invoice['CSInvoice']['amount_vat'], 2, ',', ' ') . ' Kč', 0, 1, 'R', false);
...
echo $tcpdf->Output($invoice['CSInvoice']['code'] . '.pdf', 'D');
