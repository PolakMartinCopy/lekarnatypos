...
<?php
	...
	if ( isset($this->data['CSInvoice']) ){
		$hide = '';
	}
?>
<div id="search_form"<?php echo $hide?>>

	<?php echo $form->create('CSInvoice', array('url' => array('controller' => $this->params['controller'], 'action' => 'index')))?>
	<table class="left_heading">
		...
		<tr>
			<th>Číslo dokladu</th>
			<td><?php echo $this->Form->input('CSInvoice.code', array('label' => false))?></td>
			<th>Datum vystavení od</th>
			<td><?php echo $this->Form->input('CSInvoice.date_of_issue_from', array('label' => false))?></td>
			<th>Datum vystavení do</th>
			<td><?php echo $this->Form->input('CSInvoice.date_of_issue_to', array('label' => false))?></td>
		</tr>
		<tr>
			<th>Obchodník</th>
			<td><?php echo $this->Form->input('CSInvoice.user_id', array('label' => false, 'empty' => true))?></td>
			<th>Datum splatnosti od</th>
			<td><?php echo $this->Form->input('CSInvoice.due_date_from', array('label' => false))?></td>
			<th>Datum splatnosti do</th>
			<td><?php echo $this->Form->input('CSInvoice.due_date_to', array('label' => false))?></td>
		</tr>
		<tr>
			<th>Kód skupiny</th>
	...
	<?php
		echo $form->hidden('CSInvoice.search_form', array('value' => 1));
