...
class OrderedBehavior extends ModelBehavior {
    ...
    function afterdelete(&$Model) {
        if ($Model->data) {
            ...
            $conditions = array($this->settings[$Model->alias]['field'] . ' >' => $old_weight);
            if ($this->settings[$Model->alias]['foreign_key']) {
                $conditions[$this->settings[$Model->alias]['foreign_key']] = $Model->data[$Model->alias][$this->settings[$Model->alias]['foreign_key']];
            }
    ...
    function moveto(&$Model, $id = null, $new_weight = null) {
        ...
        if ($new_weight == $old_weight) {
            ...
            $action = array($this->settings[$Model->alias]['field'] => '`' . $this->settings[$Model->alias]['field'] . '`' . ' - 1');
            $conditions[$this->settings[$Model->alias]['field'] . ' <='] = $new_weight; 
            $conditions[$this->settings[$Model->alias]['field'] . ' >' ] = $old_weight;
    ...
    function sortby(&$Model, $order, $foreign_key = null) {
        ...
        if ($this->settings[$Model->alias]['foreign_key']) {
            ...
            $conditions = array($this->settings[$Model->alias]['foreign_key'] => $foreign_key);
        }
    ...
    function moveup(&$Model, $id = null, $number = 1) {
        ...
        if (is_numeric($number)) {
            ...
            if ($this->settings[$Model->alias]['foreign_key']) {
                $conditions = array( 
                    $this->settings[$Model->alias]['field'] . ' <' => $old_weight, 
                    $this->settings[$Model->alias]['foreign_key'] =>  $Model->data[$Model->alias][$this->settings[$Model->alias]['foreign_key']]
                );
            } else {
                $conditions = array( $this->settings[$Model->alias]['field'] . ' <' => $old_weight);
    ...
    function _previous(&$Model) {
        $conditions = array($this->settings[$Model->alias]['field'] => $Model->data[$Model->alias][$this->settings[$Model->alias]['field']] - 1);
        $fields = array($Model->primaryKey, $this->settings[$Model->alias]['field']);
        if ($this->settings[$Model->alias]['foreign_key']) {
            $conditions[$this->settings[$Model->alias]['foreign_key']] = $Model->data[$Model->alias][$this->settings[$Model->alias]['foreign_key']];
            $fields[] = $this->settings[$Model->alias]['foreign_key'];                    
