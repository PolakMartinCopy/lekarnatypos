...
<table class="tabulkaedit">
	...
	<tr>
		<td>Počet kupónů</td>
		<td><?php echo $this->Form->input('DiscountCoupon.count', array('label' => false, 'size' => 3))?></td>
	</tr>
<?php
...
<script>
	$(function() {
		...
		// sprava poli pro uzivatele
		$(document).delegate('.last-customer', 'focus', function(e) {
			// zjistit cislo posledniho radku
			var row = $(this).closest('tr');
			customerCounter = $(row).attr('rel-counter');
			customerCounter++;
			// odstranim tridu last
			$(this).removeClass('last-customer');
			// pridat novy s vyssim cislem za posledni radek tridy customer-row
			var customerRow = '<tr class="customer-row" rel-counter="' + customerCounter + '">';
			customerRow += '<td>Zákazník</td>';
			customerRow += '<td>';
			customerRow += '<input name="data[DiscountCouponsCustomer][' + customerCounter + '][customer_name]" type="text" size="60" id="DiscountCouponsCustomer' + customerCounter + 'CustomerName" class="last-customer customer-name"/>';
			customerRow += '<input type="hidden" name="data[DiscountCouponsCustomer][' + customerCounter + '][customer_id]" id="DiscountCouponsCustomer' + customerCounter + 'CustomerId">';
			customerRow += '<a href="#" class="remove-row">Zrušit</a>';
			customerRow += '</td>';
			customerRow += '</tr>';
			$('.customer-row').last().after(customerRow);
		});
		...
		$(document).delegate('.remove-row', 'click', function(e) {
			...
			var row = $(this).closest('tr');
			var input = $(row).find('.customer-name');
			var isLast = false;
			if (input.hasClass('last-customer')) {
				isLast = true;
			}
			...
			if (isLast) {
				var last = null;
				var maxCounter = 0;
				$('.customer-row').each(function(index, value) {
				...
				$(last).find('.customer-name').addClass('last-customer');
			}
		});
		...
		$(document).delegate('.last-category', 'focus', function(e) {
			...
			productRow += '<input type="hidden" name="data[DiscountCouponsCategory][' + categoryCounter + '][category_id]" id="DiscountCouponsCategory' + categoryCounter + 'CategoryId">';
			productRow += '<a href="#" class="remove-row">Zrušit</a>';
			productRow += '</td>';
			productRow += '</tr>';
